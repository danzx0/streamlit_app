...
class Dspi:
    ...
    def list_to_tuple(self, x):
        return tuple(self.list_to_tuple(item) if isinstance(item, list) else item for item in x)

    # タプルからリストへ変換 O(NlogN)
    def tuple_to_list(self, x):
    ...
    def get_DSPI_free(self, input_list):
        ...
        tuple_Arcs = self.list_to_tuple(list_Arcs)
        ...
        for S in c:
            ...
            if flag:
                ...
                next_z_star[S] = [[(), -1] for i in range(self.n)]
                # print(next_z_star[S])
                for i in range(self.n - 1):
        ...
        for k in range(budget-1, -1, -1):
            ...
            for S in c:
                z_bar[S] = [0] * self.n # continueしたものは0が代入される
                next_z_bar[S] = [[(), -1] for i in range(self.n)]
                next_z_star[S] = [[(), -1] for i in range(self.n)]
                list_S = self.tuple_to_list(S)
                # print('list_S')
                # print(list_S)
                for i in range(self.n):
                    ...
                    if num > 0:
                        ...
                        for j in range(1, 2**num):
                            ...
                            if  len(new_S) > budget:
                                continue
                            new_S.sort()
                            new_S = self.list_to_tuple(new_S)
                            ...
                            for list_arc in G[i]:
                                # c_tildeの更新
                                arc = self.list_to_tuple(list_arc)
                                if arc in new_S:
                                    c_tilde = arc[2] + arc[3]
                                else:
                                    c_tilde = arc[2]
                ...
                l = [self.INF] * self.n
                l[t] = 0
                l_caron = [self.INF] * self.n
                l_caron[t] = 0
                l_Q = [(0, t)]
                for i in range(t):
                    heappush(l_Q, (self.INF, i))
                # すでに訪れたかどうかを保持しておく
